...
import { useApp, useAuth } from '../context.jsx';
...
export default function PostUpdate() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    addUpdate({ ...form, author: user.name, role: user.role });
  ...
  return (
    <div className="mx-auto max-w-3xl rounded-3xl bg-white p-6 shadow-sm border border-civic-200">
      <h2 className="text-2xl font-bold text-civic-800 mb-4">Post an Update</h2>
      {submitted ? (
        <div className="rounded-3xl bg-civic-50 p-5 text-civic-700 border border-civic-200">
          Your update is now visible to platform users.
        </div>
      ) : (
